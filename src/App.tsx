...
const initialTeams: Team[] = [
  { id: 1, drawOrder: 1, name: 'A Infinite', color: '#8a68ff', female: '女子选手 A', male: '男子选手 A', duo: '双人选手 A1 / A2', scores: { women: scoreSet(3), men: scoreSet(3), duo: scoreSet(3), team: scoreSet(6) } },
  { id: 2, drawOrder: 2, name: 'B Infinite', color: '#26d9bd', female: '女子选手 B', male: '男子选手 B', duo: '双人选手 B1 / B2', scores: { women: scoreSet(3), men: scoreSet(3), duo: scoreSet(3), team: scoreSet(6) } },
  { id: 3, drawOrder: 3, name: 'C Infinite', color: '#ffbc4d', female: '女子选手 C', male: '男子选手 C', duo: '双人选手 C1 / C2', scores: { women: scoreSet(3), men: scoreSet(3), duo: scoreSet(3), team: scoreSet(6) } },
  { id: 4, drawOrder: 4, name: 'D Infinite', color: '#f264b6', female: '女子选手 D', male: '男子选手 D', duo: '双人选手 D1 / D2', scores: { women: scoreSet(3), men: scoreSet(3), duo: scoreSet(3), team: scoreSet(6) } },
  { id: 5, drawOrder: 5, name: 'E Infinite', color: '#5caaff', female: '女子选手 E', male: '男子选手 E', duo: '双人选手 E1 / E2', scores: { women: scoreSet(3), men: scoreSet(3), duo: scoreSet(3), team: scoreSet(6) } },
  { id: 6, drawOrder: 6, name: 'F Infinite', color: '#fa7c57', female: '女子选手 F', male: '男子选手 F', duo: '双人选手 F1 / F2', scores: { women: scoreSet(3), men: scoreSet(3), duo: scoreSet(3), team: scoreSet(6) } },
  { id: 7, drawOrder: 7, name: 'G Infinite', color: '#8bc34a', female: '女子选手 G', male: '男子选手 G', duo: '双人选手 G1 / G2', scores: { women: scoreSet(3), men: scoreSet(3), duo: scoreSet(3), team: scoreSet(6) } },
...
const stageTotal = (team: Team, stage: StageKey) => team.scores[stage].reduce((total, score) => total + score, 0)
...
function Control({ teams, setTeams, publish }: { teams: Team[], setTeams: Dispatch<SetStateAction<Team[]>>, publish: () => void }) {
  ...
  return <main className="control-page"><section className="control-content team-control">
    <header className="control-header"><div><p>赛事控制台 · 团队总积分赛</p><h1>2026 Arena Spin-X Tournament</h1></div><button className="display-button" onClick={() => window.open(`${window.location.pathname}?view=display`, '_blank', 'noopener,noreferrer')}><ExternalLink size={18} />打开观赛大屏</button></header>
    <div className="format-note"><b>赛制说明</b><span>每队 4 人：女子个人、男子个人、双人舞台及团体舞台。各项目分数直接累计为团队总分。</span></div>
    ...
    <section className="panel info-panel publish-panel"><span className="eyebrow">当前录入 · {stage.name}</span><strong>草稿不会自动显示</strong><p>完成任一项目评分后，点击发布，观赛大屏会按四个项目的累计总分重新排名。</p><button className="publish-button" onClick={publish}>更新总积分至观赛大屏</button><button className="reset-button" onClick={() => setTeams(current => current.map(team => ({ ...team, submittedAt: undefined, scores: { women: scoreSet(3), men: scoreSet(3), duo: scoreSet(3), team: scoreSet(6) } })))}>重置所有分数</button></section>
    <section className="panel scoring-panel"><div className="panel-heading"><span>{stage.name} · 评分录入</span><div className="score-actions"><small>拖动团队左侧把手调整顺序；{stage.judges} 位评审，单项直接相加</small><button className="add-player" onClick={() => setAdding(true)}><UserPlus size={15} />添加团队</button></div></div><div className="stage-table"><div className="stage-head"><span>团队</span><span>{stageKey === 'team' ? '参赛团队' : '本项目参赛者'}</span><span>评审打分</span><span>本项目总分</span></div>{teams.map(team => <div className={`stage-row ${draggedTeamId === team.id ? 'dragging' : ''} ${dropTarget?.teamId === team.id ? `drop-${dropTarget.position}` : ''}`} key={team.id} draggable onDragStart={() => setDraggedTeamId(team.id)} onDragEnd={() => { setDraggedTeamId(null); setDropTarget(null) }} onDragOver={event => { event.preventDefault(); const bounds = event.currentTarget.getBoundingClientRect(); setDropTarget({ teamId: team.id, position: event.clientY < bounds.top + bounds.height / 2 ? 'before' : 'after' }) }} onDrop={() => { if (draggedTeamId && dropTarget) moveTeam(draggedTeamId, team.id, dropTarget.position); setDraggedTeamId(null); setDropTarget(null) }}><div className="team-mini"><span className="drag-handle" title="拖动调整团队顺序"><GripVertical size={17} /></span><i style={{ background: team.color }} /><strong>{team.name}</strong></div><div className="member-name">{team[stage.member]}</div><div className="judge-inputs">{team.scores[stageKey].map((score, index) => <div className="score-control" key={index}><button onClick={() => updateScore(team.id, index, score - 1)}><Minus size={13} /></button><ScoreInput score={score} label={`${team.name} 第 ${index + 1} 位评审`} onChange={value => updateScore(team.id, index, value)} /><button onClick={() => updateScore(team.id, index, score + 1)}><Plus size={13} /></button></div>)}</div><strong className="control-total">{stageTotal(team, stageKey)}</strong><div className="team-row-actions"><button onClick={() => setEditingTeam(team)} aria-label="编辑团队"><Pencil size={14} /></button><button onClick={() => removeTeam(team)} aria-label="删除团队"><Trash2 size={14} /></button></div></div>)}</div></section>
